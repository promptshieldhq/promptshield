...
Set these values in `apps/server/.env` for external proxy/engine connectivity:
...
PROXY_LOG_SOURCE=file
PROXY_LOG_FILE=../../proxy-logs/promptshield.log
```

If your proxy pushes audit events directly, set this in `promptshield-gateway/.env`:
