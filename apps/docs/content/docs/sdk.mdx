...
description: Point your existing OpenAI SDK at the gateway. One line change.
---

PromptShield accepts the same request format as OpenAI's `/v1/chat/completions`. Point your SDK's `base_url` at the gateway. Nothing else changes.
...
Instead of storing keys in `.env` on the gateway server, pass them per-request via headers. The gateway checks these before falling back to environment variables:
