...
## Reaching the gateway from Docker

The compose file uses `host.docker.internal` so Prometheus can scrape the gateway on your host. Works on Docker Desktop (Mac, Windows) and Linux with Docker Engine 20.10+.

If the gateway is on a different host, edit `infra/observability/prometheus.yml`:
